...
import {
  ...
  handleTaskHierarchyResource,
  handleActiveTasksResource,
  handleArchivedTasksResource,
} from './task-resources.js';
import {
  handleQueueResource,
  handleQueueSummaryResource,
  handleQueueListResource,
  handleQueueStatsResource,
  handleQueueTasksResource,
} from './queue-resources.js';
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (parsed.type) {
        ...
        case 'task_hierarchy':
          return await handleTaskHierarchyResource(taskService, parsed.params as { id: string });
        ...
        case 'queue_list':
          return await handleQueueListResource(queueService);

        case 'queue_stats':
          return await handleQueueStatsResource(
            queueService,
            parsed.params as { queue_name: string }
          );

        case 'queue_tasks':
          return await handleQueueTasksResource(
            queueService,
            parsed.params as { queue_name: string }
          );
